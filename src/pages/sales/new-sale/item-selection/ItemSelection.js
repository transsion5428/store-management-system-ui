...
const ItemSelection = ({ onSelectionChange }) => {
    ...
    const handleCheckboxChange = (article, isChecked) => {
        if (isChecked) {
            ...
            const updated = articles.filter(a => a.articleId !== article.articleId);
            setArticles(updated);
            onSelectionChange(updated);
        }
    }
                            ...
                            {paginator.articles && paginator.articles.length > 0 ? (
                                paginator.articles.map(article => (
                                    <tr key={article.articleId}>
                                        ...
                                        <td>{article.stock}</td>
                                        <td>{article.purchasePrice.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</td>
                                        <td>{article.salePrice.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</td>
                            ...
                            <tbody>
                                {articles.map(article => (
                                    <tr key={article.articleId}>
                                        ...
                                        <td>{article.salePrice.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</td>
                                        <td>{(article.salePrice * article.quantity).toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</td>
                                        <td>
                                            <FontAwesomeIcon
                                                icon={faTrashCan}
